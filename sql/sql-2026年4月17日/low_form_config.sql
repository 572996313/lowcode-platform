...
(
    id                 bigint auto_increment comment '表单ID'
        primary key,
    template_id        bigint                                null comment '关联的模板ID',
    source_type        varchar(20) default 'custom'          null comment '来源类型（template=模板 custom=自定义）',
    form_name          varchar(100)                          not null comment '表单名称',
    form_code          varchar(100)                          null comment '表单编码',
    table_id           bigint                                null comment '关联数据库表ID（关联db_table.id）',
    style_template_id  bigint                                null comment '样式模板ID',
    template_code      varchar(50)                           null comment '表单模板编码',
...
create index idx_table_id
    on low_form_config (table_id);

create index idx_template_id
    on low_form_config (template_id);

INSERT INTO lowcode_platform.low_form_config (template_id, source_type, form_name, form_code, table_id, style_template_id, template_code, form_type, layout_type, layout_cols, label_width, label_position, size, config_json, rules_json, status, deleted, create_by, create_time, update_time, component_category, component_tags) VALUES (null, 'custom', '通用新增表单', 'common_add_form', null, null, null, 'add', 'horizontal', 2, 100, 'right', 'default', null, null, 1, 0, null, '2026-01-31 08:48:51', '2026-01-31 08:48:51', 'common', '["system","create"]');
