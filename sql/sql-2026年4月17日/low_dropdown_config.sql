...
INSERT INTO lowcode_platform.low_dropdown_config (config_name, config_code, dropdown_type, sql_template, label_field, value_field, parent_field, param_config, extra_fields, description, enabled, sort_order, created_by, create_time, update_time, deleted) VALUES ('部门树', 'dept_tree', 'tree', 'SELECT id, dept_name, parent_id FROM sys_dept WHERE status = 1 ORDER BY sort_order, id', 'dept_name', 'id', 'parent_id', '{"params": []}', '', '部门树形下拉', 1, 1, 'system', '2026-02-01 19:57:01', '2026-02-01 19:57:01', 0);
INSERT INTO lowcode_platform.low_dropdown_config (config_name, config_code, dropdown_type, sql_template, label_field, value_field, parent_field, param_config, extra_fields, description, enabled, sort_order, created_by, create_time, update_time, deleted) VALUES ('用户状态', 'user_status', 'flat', 'SELECT 
...
INSERT INTO lowcode_platform.low_dropdown_config (config_name, config_code, dropdown_type, sql_template, label_field, value_field, parent_field, param_config, extra_fields, description, enabled, sort_order, created_by, create_time, update_time, deleted) VALUES ('带参数的用户列表', 'user_list_by_status', 'flat', 'SELECT 
  id as value,
  username as label,
  dept_name,
  email
